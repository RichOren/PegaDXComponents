...
export const inspectCaseSummaryAndReturnList = async (pConn: any, paramDataPage: string ) => {
    ...
    try {
        ...
        arrayOfCustomerList.map((listitem: any) => (
                        rowArray.push({
                            id: `${tableId+arrayOfCustomerList.selectedRowIndex}`, // This is must to have for unique id for each row
...
export const getDisbursementEmbeddedData = async (paramPConn: any, paramEmbedName: string ) => {
    try {
        ...
        const caseSummaryContentObj = caseSummaryObj.content;
        console.log('caseSummaryContentObj', caseSummaryContentObj);
        console.log('paramEmbedName', paramEmbedName);
        ...
        arrayOfEmbeddedList?.map((listitem: any) => {
            rowArray.push({
                id: generateRandomId(), // Add a random ID
...
export const getDisbursementDataAsRowData = (dataPageResults: []) => {
    ...
    dataPageResults.map(arrayData => (
        rowArray.push({
            ...
            detailsData: [
                { id: `${tableId  }1`, item: 'detailsField1', amt: '100' },
                { id: `${tableId  }2`, item: 'detailsField2', amt: '100' },
                { id: `${tableId  }3`, item: 'detailsField3', amt: '100' },
                { id: `${tableId  }4`, item: 'detailsField4', amt: '100' },
                { id: `${tableId  }5`, item: 'detailsField5', amt: '100' },
            ],
        })
    ))
    console.log(rowArray);
